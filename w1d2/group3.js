...
function insertionSort(arr) {
    var currentVal;
    for(var i = 1; i < arr.length; i++){ // initial loop
    currentVal = arr[i];
        for(var j = i - 1; j >= 0 && arr[j] > currentVal; j--) { // 2nd loop, running from opposite side of first loop
            arr[j + 1] = arr[j]; // when loop ends, correct insertion spot found
        }
        arr[j + 1] = currentVal; // the insertion of currentVal into its right spot
    }
    return arr;
}
